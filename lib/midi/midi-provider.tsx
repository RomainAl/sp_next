...
import { accessReceived, defaultState, reducer, ReducerState } from "./reducer";
...
export const MIDIProvider = ({ children }: Props) => {
  ...
  useEffect(() => {
    ...
    if (selectedInput) {
      selectedInput.onmidimessage = emitter.current.onMIDIMessage.bind(emitter.current);
      return () => (selectedInput.onmidimessage = null);
    }
    return () => {};
  }, [inputs, selectedInputId]);

  return <MIDIContext.Provider value={{ state, dispatch, emitter: emitter.current }}>{children}</MIDIContext.Provider>;
};
