...
export const reducer = (state: ReducerState = defaultState, action: { type: string; payload?: any }): ReducerState => {
  ...
  switch (type) {
    case ActionTypes.accessReceived:
      ...
      const inputExists = payload.inputs.some((input: Input) => input.id === state.selectedInputId);
      const outputExists = payload.outputs.find((output: Input) => output.id === state.selectedOutputId);
      return {
        ...state,
        inputs: payload.inputs,
        outputs: payload.outputs,
        selectedInputId: inputExists ? state.selectedInputId : payload.inputs.length ? payload.inputs[0].id : null,
        selectedOutputId: outputExists ? state.selectedOutputId : payload.outputs.length ? payload.outputs[0].id : null,
      };
