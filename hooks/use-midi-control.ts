...
import { Message, MIDIControl, MIDIControlFilter } from "../lib/midi/types";
...
export const useMIDIControl = ({ cc: controlFilter, channel: channelFilter }: MIDIControlFilter = {}) => {
  ...
  const handleCC = useCallback(
    (message: Message) => {
      ...
      if ((!controlFilter || controlFilter === target) && (!channelFilter || channelFilter === channel)) {
