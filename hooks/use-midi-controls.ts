...
import { MIDIControlFilter } from "../lib/midi/types";
...
export const useMIDIControls = (controls: number[], filter: Exclude<MIDIControlFilter, "cc"> = {}) => {
  ...
  useEffect(() => {
    if (cc) {
      ...
      if (targetIndex > -1) setValues(values.map((v, i) => (i === targetIndex ? cc.value : v)));
    }
  }, [cc, controls, values]);
