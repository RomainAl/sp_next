...
import { MIDINote, MIDINoteFilter, NoteMessage } from "../lib/midi/types";
...
export const useMIDINote = ({ note: noteFilter, channel: channelFilter }: MIDINoteFilter = {}): MIDINote | undefined => {
  const [value, setValue] = useState<MIDINote | undefined>();

  const handleNote = useCallback(
    (message: NoteMessage) => {
      const { target: note, value: velocity, on, channel } = message;
      if ((!noteFilter || noteFilter === note) && (!channelFilter || channelFilter === channel)) {
