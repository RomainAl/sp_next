"use client";
import { MIDIConstants } from "../lib/midi/constants";
...
const getNoteMessages = ({ type, noteInput, velocity = 127, channel = 1 }: GetNoteMessagesArgs) => {
  ...
  if (typeof noteInput === "number") {
    ...
    return noteInput.reduce((messages, note) => {
      if (typeof note === "number") {
        ...
        messages.push(note.channel ? type | getChannel(note.channel) : defaultChannelData, note.note, note.velocity ?? velocity);
      }
      return messages;
    }, [] as number[]);
  }
